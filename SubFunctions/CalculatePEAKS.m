%% Function to analyse peaks in force 
function [peaks,peaks_contra,name] = CalculatePEAKS (signal,fS,status)

Int_SearchMaxMin=30;
     
% Filter
fx  = sgolayfilt(signal,3,21);
%fx  = signal;
...
% %trova un valore medio (nel caso ci fosse un offset) QUESTO PASSAGGIO é
% STATO SPOSTATO PRECEDENTEMENTE PER FARLO FILE PER FILE
%        
% mfx  = median(fx(status==0));
%fx = fx-mfx; 

%th = mfx+fstd;
fstd = std(fx);

th = fstd; % uncomment the previous th if you don't remove the offset

%[pks,locs] = findpeaks_GUI(-fx,'MINPEAKHEIGHT',th,'MINPEAKDISTANCE',100);
[pks,locs] = findpeaks(-fx,fS,'MINPEAKDISTANCE',0.30,'MinPeakProminence',th);
locs = round(locs*100);
pks=-pks;

%[pks_contra,locs_contra] = findpeaks_GUI(fx,'MINPEAKHEIGHT',th,'MINPEAKDISTANCE',100);
[pks_contra,locs_contra] = findpeaks(fx,fS,'MINPEAKDISTANCE',0.30,'MinPeakProminence',th);
locs_contra = round(locs_contra*100);
...
for i=1:length(pks_contra)
    ...
    %ratio_ampl(i) = pks_contra(i)-fx(onset_force_peak);
