...
DAY_SP = {'DAY00_BL_mixdays','DAY11_2019_10_07_14DPI','DAY13_2019_10_29_35DPI'};
DAY_ROB = {'DAY01_BL_2020_07_27','DAY02_Wk2_2020_08_11_14DPI','DAY03_Wk4_2020_08_28_35DPI'};
n_days = size(DAY_SP,2);

dayPlot = {'BL','2 Weeks','4 Weeks'}; 
mycolor = {[209 211 212]./255,[234,110,47]./255,[153,28,0]./255,... % Color spontaneous
    [136,141,145]./255,[174,220,219]./255,[28,147,143]./255}; % Color Rob
...
vect_allpars = cell (size(DAY_SP,2)*n_treat,1);
matrix_allpars = cell(size(DAY_SP,2)*n_treat,1); 
info_rat = cell(size(DAY_SP,2)*n_treat,1); %the number of ROB group is +100
info_day = cell(size(DAY_SP,2)*n_treat,1);
...
for i_day = 1:n_days
    ...
    if ~contains (DAY_ROB{1,i_day},'DPI') % we keep track of the day that we are analysing
        info_day{i_day+n_days,1} = 0; % all the baseline data can be analyzed together, so they are all zero
    else
        info_day{i_day+n_days,1} = str2num(DAY_ROB{1,i_day}(end-4:end-3)); % number of the day 
    end
    for i_rat = 3:size(ListRat,1)-1 % because there is always the file ReadMe in every folder
    ...
    %                     % Data EMG BI
                        ...
                        line_tot = [line1,line3,line5];
                        if sum(isnan(line_tot))<length(line_tot)/3
                               matrix_allpars{i_day+n_days,1}= [matrix_allpars{i_day+n_days,1};line1,line3,line5];
                               info_rat{i_day+n_days,1} = [info_rat{i_day+n_days};str2num(ListRat(i_rat,1).name(2:3))+100]; %to distinguish the animals of the second group we had 100
                        end
