...
DAY_SP = {'DAY00_BL_mixdays','DAY11_2019_10_07_14DPI','DAY13_2019_10_29_35DPI','DAY04_Wk8_2020_09_23_56DPI'};
DAY_ROB = {'DAY01_BL_2020_07_27','DAY02_Wk2_2020_08_11_14DPI','DAY03_Wk4_2020_08_28_35DPI','DAY04_Wk8_2020_09_23_56DPI'};
n_days = size(DAY_SP,2);
n_days_rob = size(DAY_ROB,2);

dayPlot = {'BL','2 Weeks','4 Weeks','8 Weeks'}; 
mycolor = {[209 211 212]./255,[234,110,47]./255,[153,28,0]./255,[0,0,0]./255,... % Color spontaneous
    [136,141,145]./255,[174,220,219]./255,[28,147,143]./255,[14,107,103]./255}; % Color Rob
...
datapath_SP = 'C:\R-Platform\DATA\2019_02_Group1\Recordings_robot\';
datapath_ROB = 'C:\R-Platform\DATA\2020_08_RRR_group\Robot Data\';
filename_ROBgroup = 'C:\R-Platform\DATA\2020_08_RRR_group\Code_yRRRgroup\Groups\all_groups.txt';
Path = cd;
...
vect_allpars = cell (size(DAY_ROB,2)*n_treat,1);
matrix_allpars = cell(size(DAY_ROB,2)*n_treat,1); 
info_rat = cell(size(DAY_ROB,2)*n_treat,1); %the number of ROB group is +100
info_day = cell(size(DAY_ROB,2)*n_treat,1);

%% load data SP GROUP

for i_day = 1:n_days
    ListRat = dir ([datapath_SP,DAY_SP{1,i_day}]); 
    if ~contains (DAY_SP{1,i_day},'DPI') % we keep track of the day that we are analysing
        info_day{i_day,1} = 0; % all the baseline data can be analyzed together, so they are all zero
    else
        info_day{i_day,1} = str2num(DAY_SP{1,i_day}(end-4:end-3)); % number of the day 
    end
    for i_rat = 3:size(ListRat,1)-1 % because there is always the file ReadMe in every folder
        ListFile = dir([datapath_SP,DAY_SP{1,i_day},'\',ListRat(i_rat,1).name]);
        in_files = [];
        ...
        for i_file_ok = 1:length(in_files)
            load ([datapath_SP,DAY_SP{1,i_day},'\',ListRat(i_rat,1).name,'\',ListFile(in_files(i_file_ok),1).name]);
...
%% load data ROB GROUP
fpt = fopen(filename_ROBgroup);  % Open programming students grade file
namesROB = fscanf(fpt, '%s',[1 inf]); %

for i_day = 1:n_days_rob
    ListRat = dir ([datapath_ROB,DAY_ROB{1,i_day}]); 
    if ~contains (DAY_ROB{1,i_day},'DPI') % we keep track of the day that we are analysing
        info_day{i_day+n_days_rob,1} = 0; % all the baseline data can be analyzed together, so they are all zero
    else
        info_day{i_day+n_days_rob,1} = str2num(DAY_ROB{1,i_day}(end-4:end-3)); % number of the day 
    end
    for i_rat = 3:size(ListRat,1)-1 % because there is always the file ReadMe in every folder
        % check rat in the list               
        if contains (namesROB,ListRat(i_rat,1).name)
            ListFile = dir([datapath_ROB,DAY_ROB{1,i_day},'\',ListRat(i_rat,1).name]);
            in_files = [];
            ...
            for i_file_ok = 1:length(in_files)
                load ([datapath_ROB,DAY_ROB{1,i_day},'\',ListRat(i_rat,1).name,'\',ListFile(in_files(i_file_ok),1).name]);
                if contains(Data.info.Status,'Push_active')
    ...
    %                     % Data EMG BI
                        ...
                        line_tot = [line1,line3,line5];
                        if sum(isnan(line_tot))<length(line_tot)/3
                               matrix_allpars{i_day+n_days_rob,1}= [matrix_allpars{i_day+n_days_rob,1};line1,line3,line5];
                               info_rat{i_day+n_days_rob,1} = [info_rat{i_day+n_days_rob};str2num(ListRat(i_rat,1).name(2:3))+100]; %to distinguish the animals of the second group we had 100
                        end
...
if NORM
    ...
    if MEDrat
        matrix_all_medrat = NaN(length(ListRat),size(NameParam,2),n_days_rob);
       for j_rat = 1:size(matrix_all_medrat,1)
           n_rat = ListRat(j_rat);
           for j_day = 1 : n_days_rob
               if ~contains (DAY_SP{1,j_day},'DPI') % we keep track of the day that we are analysing
                   n_day = 0; % all the baseline data can be analyzed together, so they are all zero
               else
                   n_day = str2num(DAY_SP{1,j_day}(end-4:end-3)); % number of the day
               end
       ...
       max_cycles = NaN(n_days_rob,1);
       for j_day = 1 : n_days_rob
           max_cycles(j_day) = length(find(info_data(:,1) == tot_days(j_day)));
       end
       MM = NaN(max(max_cycles),size(NameParam,2),n_days_rob);
       for j_day = 1 : n_days_rob
...
nt_d = n_days_rob; %total number of days (baseline all together)
n_gr = [sum(ListRat<100); sum(ListRat>100 & ListRat<200)];
xbar = [1:1:nt_d];
...
if PLOT_p    
    for j_par = 1: nt_p
        ...
        if ~NORM
            for jj = 1 :nt_d
                x_stat = [x_stat;MM(1:8,j_par,jj)];
                group = [group;ones(8,1)*jj];
            end
            %statistic
            ...
            group = [];
            for jj = 1 :nt_d
                x_stat = [x_stat;MM(9:end,j_par,jj)];
                group = [group;ones(length(MM(9:end,j_par,jj)),1)*jj];
            end
            %statistic
            [p,tbl,stats] = kruskalwallis(x_stat,group);
...
%nanstd(matrix_allparams_allanimals_z,1);
 ...
 for pl_day = 1:nt_d
     if pl_day==1 % we keep track of the day that we are analysing
           ...
           n_day = str2num(DAY_SP{1,pl_day}(end-4:end-3)); % number of the day
     end
    ...
    for i_rat = 1:size(xp,1)
     ...
     x_r = xp;
     y_r = yp;
     scatter(nanmean(x_r(gr==1)),nanmean(y_r(gr==1)),200,'MarkerFaceColor', mycolor{pl_day},'MarkerEdgeColor','k','DisplayName',[dayPlot{pl_day},'-SP'])
...
if PLOT3
    ...
    for pl_day = 1:nt_d
        ...
        y_r = yp;
        z_r = zp;
        scatter3(nanmean(x_r(gr==1)),nanmean(y_r(gr==1)),nanmean(z_r(gr==1)),200,'MarkerFaceColor', mycolor{pl_day},'MarkerEdgeColor','k','DisplayName',[dayPlot{pl_day},'-SP'])
        hold on
